...
#pragma mark SimpleVertexShader
                   ...
                   const device AAPLSimpleVertex *vertices [[ buffer(AAPLVertexInputIndexVertices) ]])
{
    ...
    out.pos=vertices[vertexID].position.xyz;
    return out;
}
...
#pragma mark texture Vertes Sheader
vertex TexturePipelineRasterizerData
textureVertexShader(const uint vertexID [[ vertex_id ]],
                    const device AAPLTextureVertex3D *vertices [[ buffer(AAPLVertexInputIndexVertices) ]],
                    constant float &aspectRatio [[ buffer(AAPLVertexInputIndexAspectRatio) ]])
{
    TexturePipelineRasterizerData out;

    out.position = vector_float4(out.position.xyz, 1.0);
    ...
    out.texcoord.xy = vertices[vertexID].texcoord.xy;

    return out;
}
// Fragment shader that samples a texture and outputs the sampled color.
#pragma mark Chessboard generator
